...
class ConversationStore:
    ...
    def setup(self) -> None:
        with self._connect() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_messages_session_id ON conversation_messages(session_id, id)"
            )
            conn.execute(
                """
    ...
    def clear_long_term_memories(self, user_id: str) -> None:
        with self._connect() as conn:
    ...
    def append_turn(self, session_id: str, user_query: str, assistant_answer: str, scene: str) -> None:
    ...
    def get_recent_messages(self, session_id: str, recent_rounds: int) -> list[ConversationMessage]:
    ...
    def get_active_long_term_memories(
        self,
        user_id: str,
        limit: int = 5,
    ) -> list[LongTermMemory]:
    ...
    def mark_long_term_memories_used(self, memory_ids: list[int]) -> None:
        if not memory_ids:
            return
    ...
    def deactivate_long_term_memory(self, memory_id: int, user_id: str) -> None:
        now = datetime.now().isoformat(timespec="seconds")
        with self._connect() as conn:
            conn.execute(
                """
